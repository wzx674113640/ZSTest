...
gulp.task("copyLibsJsFile", ["copyFile"], function () {
	...
	// 微信、百度，需要拷贝对应平台的类库
	if (platform === "wxgame") {
		copyLibsList.push(`${workSpaceDir}/bin/libs/laya.wxmini.js`,`${workSpaceDir}/bin/libs/zs.laya.wm.adapter.js`);
...
BK.Script.loadlib("GameRes://index.js");`;
	...
	if (platform === "wxgame") {
		...
		if (fileContent.includes("//-----libs-end-------")) {
			fileContent = fileContent.split("//-----libs-end-------").join(`\nloadLib("libs/zs.laya.wm.adapter.js")\n//-----libs-end-------`);
		}
		fs.writeFileSync(filePath, fileContent, "utf8");
		return;
	}
